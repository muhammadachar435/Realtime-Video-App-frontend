...
const RoomPage = () => {
  ...
  const isProcessingPendingCall = useRef(false); // Add this flag
  ...
  // ------------------ FIXED: Process Pending Call ------------------
  const processPendingCall = useCallback(async () => {
    if (isProcessingPendingCall.current || !pendingIncomingCall.current) {
      return;
    }

    isProcessingPendingCall.current = true;
    
    try {
      const pendingCall = pendingIncomingCall.current;
      console.log("🔄 Processing pending call:", pendingCall);
      
      // Clear pending call immediately to prevent re-processing
      const callToProcess = { ...pendingCall };
      pendingIncomingCall.current = null;
      
      // Call handleIncomingCall
      await handleIncomingCall({
        from: callToProcess.from,
        offer: callToProcess.offer,
        fromName: callToProcess.fromName
      });
    } catch (error) {
      console.error("❌ Error processing pending call:", error);
    } finally {
      isProcessingPendingCall.current = false;
    }
  }, [handleIncomingCall]);
  ...
  }, [sendStream, applyEnhancedEchoCancellation]);
  ...
  // ------------------ FIXED: Process pending call when stream is ready ------------------
  useEffect(() => {
    if (state.streamReady && pendingIncomingCall.current && !isProcessingPendingCall.current) {
      console.log("✅ Stream ready, processing pending call...");
  ...
  }, [state.streamReady, processPendingCall]);
  ...
  return (
    ...
    `}
      ...
      {/* Connection Status */}
      {pendingIncomingCall.current && state.streamReady && (
